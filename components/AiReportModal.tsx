                ...
                {sortedClassIds.map(classId => (
                    <div key={classId} className="w-52 bg-white rounded-lg border border-slate-200 flex flex-col shadow-sm flex-shrink-0">
                        <div className="p-2 border-b border-slate-100 bg-slate-50/50 rounded-t-lg flex justify-between items-center">
                            <span className="font-bold text-sm text-slate-800">{classId}반</span>
                            <span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full text-slate-700 font-bold">{classes[classId].length}명</span>
                        </div>
                        <div className="p-2 space-y-2">
                            {classes[classId].length === 0 && (
                                <div className="text-center text-xs text-gray-400 py-4">학생 없음</div>
                            )}
                                ...
                                return (
                                    ...
                                    `}>
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {s.gender && (
                                                <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${s.gender === 'male' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                                    {s.gender === 'male' ? '남' : '여'}
                                                </span>
                                            )}
                                            {s.tagIds.map(tid => {
                                                ...
                                                return t ? (
                                                    <span key={tid} className={`text-[10px] px-1.5 py-0.5 rounded border border-transparent font-medium ${t.colorBg} ${t.colorText}`}>
                                                        {t.label}
                                                    </span>
                                                ) : null;
                                            })}
...
export const AiReportModal: React.FC<AiReportModalProps> = ({ 
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
               <div className="prose prose-sm max-w-none text-gray-700 whitespace-pre-line leading-relaxed bg-white p-6 rounded-xl shadow-sm border border-purple-100">
                   {renderMarkdown(analysisResult)}
               </div>
            </div>
            <div className="p-4 border-t bg-white flex justify-end gap-3">
              <button onClick={onClose} className="px-5 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg font-bold transition-colors text-sm">닫기</button>
              {onReanalyze && (
                  <button 
                    onClick={onReanalyze}
                    className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold transition-colors flex items-center gap-2 text-sm"
                  >
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div 
        className="bg-white w-full max-w-6xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 p-6 space-y-6">
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col items-center justify-center relative overflow-hidden min-h-[200px]">
                    ...
                    <h3 className="text-gray-500 font-bold mb-4 uppercase tracking-wider text-sm">현재 전체 균형 점수</h3>
                ...
                {/* Comment Card */}
                <div className="md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-200 relative flex flex-col justify-center">
                     <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-purple-500 to-pink-500"></div>
                     <h3 className="text-gray-500 font-bold mb-4 uppercase tracking-wider text-sm flex items-center gap-2">
                        <TrendingUp size={18} /> 종합 평가
                     </h3>
                     <div className="flex-1 flex items-center">
                        <p className="text-gray-800 leading-relaxed text-lg font-semibold break-keep">
                            "{overallReview}"
                        </p>
                     </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="w-full h-72">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={barChartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                ...
                                <Bar dataKey="Risk" name="지도 난이도(Risk)" fill="#f87171" radius={[4, 4, 0, 0]} barSize={32} />
                                <Bar dataKey="Balance" name="균형 점수(Balance)" fill="#60a5fa" radius={[4, 4, 0, 0]} barSize={32} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </section>
                ...
                <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 h-full flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col gap-3 overflow-y-auto max-h-[300px] lg:max-h-none pr-2">
                        ...
                        {classBriefs.length === 0 && (
                            <div className="text-center text-gray-400 py-6 flex-1 flex items-center justify-center text-sm">특별한 제안 사항이 없습니다.</div>
                        )}
                    </div>
                </section>
            </div>
                    ...
                    {classDetails.map((cls) => (
                        <div key={cls.classId} className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <h5 className="text-xs font-bold text-gray-400 uppercase mb-1.5">현황</h5>
                                    <p className="text-sm text-gray-700 leading-relaxed">{cls.currentSituation}</p>
                                </div>
                                <div>
                                    <h5 className="text-xs font-bold text-gray-400 uppercase mb-1.5">긍정적 요소</h5>
                                    <p className="text-sm text-green-800 leading-relaxed bg-green-50 p-3 rounded-lg">{cls.positiveFactors}</p>
                                </div>
                                <div>
                                    <h5 className="text-xs font-bold text-gray-400 uppercase mb-1.5">조언</h5>
                                    <p className="text-sm text-amber-800 leading-relaxed bg-amber-50 p-3 rounded-lg">{cls.advice}</p>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <section className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-2xl border border-indigo-100">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div className="bg-white px-4 py-2 rounded-full border border-indigo-100 text-sm text-gray-500 shadow-sm">
                ...
                {suggestions && suggestions.length > 0 ? (
                    ...
                    <div className="text-center py-10 bg-white/50 rounded-xl border border-dashed border-gray-300">
                        <ThumbsUp className="mx-auto text-green-500 mb-3" size={36} />
                        <p className="text-gray-800 font-bold text-lg">현재 편성이 최적 상태입니다.</p>
                        <p className="text-sm text-gray-500 mt-1">추가적인 최적화 제안이 없습니다.</p>
                    </div>
                )}
            </section>
